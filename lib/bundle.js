...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.addNotes = function(onsets, pitches) {
	  const gameObj = this;
	  this.notes = populateSong(onsets, pitches);
	...
	Game.prototype.addWhiteKeys = function() {
	  ...
	  for (let i = 0; i < Constants.NUM_NOTES; i++) {
	    this.whiteKeys.push(new whiteKey({
	        pos: [75*i+350, Constants.HEIGHT*(1/2)],
	        xSize: 70,
	...
	Game.prototype.startPlaying = function(noteName) {
	  ...
	  this.lightUpNote(noteName, that);
	};

	Game.prototype.lightUpNote = function(noteName, that) {
	  this.notes.forEach( note => {
	    if (note.noteName === noteName && note.overlapsTile()) { // if you hit the right note and the note is in the right place
			  const sound = new Sound(note.freq);
			  if (!this.sounds[noteName]) {
			    this.sounds[noteName] = sound;
			  }
			  if ($('#mute-button.off').length !== 0) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	const Note = function(options) {
	  ...
	  this.id = defaultOptions.id;
		this.freq = defaultOptions.freq;
		this.time = defaultOptions.time;
	  this.glowing = false;
	};
	...
	Note.prototype.move = function (timeDelta) {
	  ...
	  //vel is how far note should move in 1/60th of a second
		// GONNA HAVE TO INVOLVE TIME SHIZ HERE DAVID DAVID DAVID DAVID
...
/***/ function(module, exports, __webpack_require__) {
	...
	const drawFakeKeys = function(ctx) {
	  ...
	  ctx.fillStyle = 'white';
	  ctx.font='50px Orbitron';
	  [ 'd', 'f', 'j', 'k'].forEach((char, idx) => {
	    ctx.fillText(char, 75*idx+350, Constants.HEIGHT*(1/2)+220);
	  });
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	// const testSong = 'c';
	// const odeToJoy = 'CDEFCEEFCDEFCDEFDEF';
	const LionSleepsTonight = 'DFJFJKJFDFJFDJFKKJKKKJFDFJFDJF';
	const lst_pitches = [
		523.25, 587.33, 659.25, 587.33, 659.25, 698.46, 659.25, 587.33, 523.25, 587.33,
		659.25, 587.33, 523.25, 659.25, 587.33, 783.99, 698.46, 659.25, 698.46, 783.99,
		698.46, 659.25, 587.33, 523.25, 587.33, 659.25, 587.33, 523.25, 659.25, 587.33]
	// const ConcerningHobbits = 'CDEGEDCEGABGEFEDCDEGEDCEGAGED';
	// const twinkleTwinkle = 'CCGGAAGFFEEDDCGGFFEEDGGFFEEDCCGGAAGFFECDDC';

	// const NoteRNG = function(prev, current) {
	//
	// 	// Only if repeating notes.
	//
	// 	var notes = ["D","F","J","K"];
	...
	// 				return notes[notes.indexOf(current) - 1];
	// 			} else {
	// 				return "F"
	// 			}
	// 		} else {
	...
	// const songMap = function(pitches) {
	// 	var max_range = Math.max.apply(null, pitches);
	// 	var min_range = Math.min.apply(null, pitches);
	// 	var bucketSize = (max_range - min_range) / 3;
	//
	// 	var notes = "";
	// 	var prev = "";
	//
	...
	// 		var note;
	// 		if(pitches[i] < bucketSize + min_range) {
	// 			note = NoteRNG(prev, "D");
	// 		} else if(pitches[i] < bucketSize * 2 + min_range) {
	// 			note = NoteRNG(prev, "F");
	// 		} else if(pitches[i] < bucketSize * 3 + min_range) {
	// 			note = NoteRNG(prev, "J");
	// 		} else {
	// 			note = NoteRNG(prev, "K");
	// 		}
	...
	const populateSong = function(onsets, pitches) {
	  const allNotes = LionSleepsTonight;
	  const notes = allNotes.split('');
	  let song = [];
	  let offset = 0;
	  for (let i = 0; i < notes.length; i++) {
			console.log(notes[i], lst_pitches[i]);
      song.push(new Note({
        id: i + 1,
        note: notes[i],
	  ...
	  $('input').on('change', event => {
			...
			$.ajax({
				...
				success:function(data) {
					console.log(data);
					that.game.songName = data.filename;
					$('.welcome').addClass('hidden');
					that.endTutorial = true;
					that.game.addNotes(data.onsets, data.pitches);
			 	  that.game.playing = true;
			 	  that.startGame();
				}
			})
	  });
